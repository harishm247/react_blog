...
<div class="mail-option mail-box contact-us-support-mail">
	...
	<div class="col-md-10 no-padding">
		<div class="inbox-head" >
			<form action="<?php echo base_url('backend/support/user_contactus'); ?>" method="get" role="form" class="form-horizontal">
					...
					<table class="responsive table_head" cellpadding="5">          
						...
						<tbody>
							<tr>
								...
								<td width="">                               
									<input type="text" value="<?php if(!empty($_GET['support_id'])){ echo $_GET['support_id']; } ?>" name="support_id" class="form-control" placeholder="Request Id No.">
								</td> 
								<td width="">                               
								...
								<td width="">                               
									<select name="userrole" class="form-control"> 
										<option value="-1" >Select</option>                   
										<option value="0" <?php if(!empty($_GET['userrole']) && $_GET['userrole']==0) echo 'selected'?>>Customer</option>
										<option value="3" <?php if(!empty($_GET['userrole']) && $_GET['userrole']==3) echo 'selected'?>>Anonymous</option>
									   <option value="1" <?php if(!empty($_GET['userrole']) && $_GET['userrole']==1) echo 'selected'?>>Seller</option> 
								...
								<td> 
									...
									<a class="btn btn-danger tooltips" href="<?php echo base_url('backend/support/user_contactus'); ?>" rel="tooltip" data-placement="top" data-original-title="Reset your notification" type="submit"> <i class="icon icon-refresh"></i></a>
								</td>
							</tr>
						</tbody>
					</table>
					...
					<tr>
						<th width="10%" class="jv no_sort">
							<div class="custom-checkbox-list">    
								...
								<select name="commonstatus" id="commonstatus" class="form-control commonstatus order-select-status-for-support">
						...
						<th width="10%" class="to_hide_phone span2">User Role</th>
						<th width="10%" class="to_hide_phone ue no_sort">User Name</th> 
						<th width="15%" class="no_sort">Subject</th>  
						<th width="20%" class="to_hide_phone ue no_sort">Message</th>                
						<th width="15%" class="to_hide_phone span2">Request Date/time</th>
						<th width="10%" class="to_hide_phone span2">Request Complete</th>
						<th width="8%" class="ms no_sort ">Actions</th>
					</tr>
				</thead>
					<tbody>
								...
								<tr class=" <?php if($unread_msg){ echo 'unread '; }else{ echo 'read '; } ?>">
									...
									<td>
										<span class="inbox-small-cells important_notify">
											...
											<i style="<?php if($row->mark_important==1){ ?>color:#c5b928;<?php } ?>" class="icon-star <?php if($row->mark_important==1){ ?> inbox-started <?php } ?>"></i> 
											</a>
										</span>   
									</td>   
									...
									<td class="to_hide_phone">
										<?php if(isset($row->mark_complete) && $row->mark_complete==0){ ?>
											<a class="label label-success label-mini tooltips" href="<?php echo base_url('backend/support/mark_complete/'.$row->support_id)?>" rel="tooltip" data-placement="top" data-original-title="Mark as complete" > Incomplete </a> 
										<?php } 
										else{ ?>
											<a class="label label-warning label-mini tooltips" href="javascript:void(0);" rel="tooltip"> Complete </a> 
										<?php }?> 
									</td>
									<td class="ms">
										<a href="<?php echo base_url().'backend/support/user_contactus_reply/'.$row->support_id.'/'.$offset?>" class="btn btn-success btn-xs tooltips" rel="tooltip" data-placement="left" data-original-title="Reply "><i class="icon-share-alt"></i> 
										</a> 
										<a href="javascript:void(0);" onclick="return confirmBox('Do you want to delete it ?','<?php echo base_url().'backend/support/delete_user_contactus/'.$row->support_id.'/'.$offset?>')" class="btn btn-danger btn-xs black tooltips" rel="tooltip" data-placement="bottom" data-original-title="Remove">                   
						...
						<?php else: ?>
							<tr >
								<th colspan="9" > <center>No support messages found.</center></th>
							</tr>
						<?php endif; ?>
					</tbody>
				</table>
	...
	jQuery(document).ready(function($) { 
		$('#commonstatus').change(function(event) {     
			...
			if(new_status==1){
				...
				var action_name = 'Complete';
			}else{
				return false;
			}
			if($("input:checkbox[name='checkstatus[]']").is(':checked')){    
				if(confirm("Do you want to "+action_name+" all!") == false){
			...
			$.post('<?php echo base_url() ?>'+'backend/support/change_all_status/support', {'status': new_status,'row_id': row_id}, function(data) {    
				if(data.status==true){  
					window.location='<?php echo base_url()?>'+'backend/support/notification_status_msg/notification';
				}else{       
					window.location.reload(true);
					return false;
				}
